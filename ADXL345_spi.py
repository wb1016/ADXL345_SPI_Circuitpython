...
class Accelerometer:
  ...
  def __init__(self, cs_pin=board.GP9, scl_pin=board.GP10, sda_pin=board.GP11, sdo_pin=board.GP12, spi_freq=5000000):
    """
  ...
  def init_spi(self):
    ...
    self.spi = busio.SPI(self.scl_pin, self.sda_pin, self.sdo_pin)
	device = SPIDevice(self.spi, cs_pin, baudrate=self.spi_freq, polarity=1, phase=1)
  ...
  def xyzbytes2g(self, buf:bytearray) -> tuple:
    ...
    acc_x, acc_y, acc_z = zip(*[struct.unpack('<HHH', buf[i:i + self.bytes_per_3axes]) for i in range(n_act_meas) if i % self.bytes_per_3axes == 0])
